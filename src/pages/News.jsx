    ...
    return (
        <div className='min-h-screen bg-[#020202] relative'>
            ...
            <section className='relative px-8 sm:px-12 md:px-20 lg:px-28 pt-40 pb-32'>
                <div className='max-w-7xl mx-auto relative z-10'>
                    ...
                    <div className='relative mb-8 sm:mb-12 lg:mb-16'>
                        <motion.h1 className='font-2 text-[clamp(3rem,16vw,10rem)] sm:text-[clamp(4rem,14vw,9rem)] lg:text-[clamp(5rem,12vw,8rem)] text-white leading-none tracking-tighter text-safe whitespace-nowrap' initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>NEWS</motion.h1>
                        <motion.div className='absolute -bottom-2 sm:-bottom-4 left-0 h-1 sm:h-2 bg-gradient-to-r from-[#DE5127] via-orange-500 to-transparent rounded-full' initial={{ width: 0 }} animate={{ width: "40%" }} transition={{ duration: 1, delay: 0.8 }} />
                    </div>
            ...
            <section className='sticky top-0 z-40 bg-[#020202]/80 backdrop-blur-xl border-y border-white/[0.05]'>
                <div className='max-w-7xl mx-auto px-4 sm:px-8 md:px-12 lg:px-20 xl:px-28 py-4 sm:py-6'>
                    <div className='flex items-center gap-2 overflow-x-auto scrollbar-hide pb-2 sm:pb-0'>
                        {filters.map((f, index) => (
                            <motion.button key={f.id} onClick={() => setFilter(f.id)} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={`relative px-4 sm:px-6 py-2 sm:py-3 rounded-full font-gs text-[10px] sm:text-[11px] font-bold uppercase tracking-wider whitespace-nowrap transition-all duration-300 ${filter === f.id ? 'text-white' : 'text-white/40 hover:text-white/70'}`}>
            ...
            <AnimatePresence mode='wait'>
                {filter === 'all' && (
                    <motion.section key="featured" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className='px-4 sm:px-8 md:px-12 lg:px-20 xl:px-28 py-12 sm:py-16 lg:py-20'>
                        <div className='max-w-7xl mx-auto'>
                            <article onClick={() => setSelectedArticle(featuredNews)} className='group relative cursor-pointer'>
                                <div className='grid lg:grid-cols-2 gap-0 rounded-2xl sm:rounded-3xl overflow-hidden bg-white/[0.02] border border-white/[0.08] group-hover:border-[#DE5127]/30 transition-all duration-700'>
                                    <div className='hidden lg:block relative aspect-[16/9] sm:aspect-[4/3] lg:aspect-auto lg:min-h-[300px] xl:min-h-[400px] overflow-hidden'>
                                        ...
                                        {featuredNews.trending && (
                                            <div className='absolute top-4 left-4 sm:top-6 sm:left-6'>
                                                <motion.span className='flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-[9px] sm:text-[10px] font-bold uppercase bg-[#DE5127] text-white' initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3, type: "spring" }}>
                                                    <svg className='w-3 h-3 sm:w-4 sm:h-4' fill='currentColor' viewBox='0 0 24 24'><path d='M16 6l2.29 2.29-4.88 4.88-4-4L2 16.59 3.41 18l6-6 4 4 6.3-6.29L22 12V6z' /></svg>
                                                    <span className='hidden sm:inline'>Trending</span>
                                                    <span className='sm:hidden'>Hot</span>
                                                </motion.span>
                                            </div>
                                        )}
                                        <div className='absolute bottom-4 left-4 sm:bottom-6 sm:left-6 flex items-center gap-2 text-white/70'>
                                            ...
                                            <span className='font-gs text-[10px] sm:text-[11px]'>{featuredNews.views} views</span>
                                        </div>
                                    </div>

                                    <div className='relative p-6 sm:p-8 lg:p-10 xl:p-12 flex flex-col justify-center'>
                                        <div className='flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 flex-wrap'>
                                            <span className='px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[9px] sm:text-[10px] font-bold uppercase' style={{ background: categoryColors[featuredNews.category].bg, color: categoryColors[featuredNews.category].text }}>{featuredNews.category}</span>
                                            <span className='w-1 h-1 rounded-full bg-white/30' />
                                            <span className='font-gs text-[10px] sm:text-[11px] text-white/40'>{featuredNews.readTime} read</span>
                                        </div>

                                        <h2 className='font-2 text-[clamp(1.75rem,5vw,3.5rem)] sm:text-[clamp(2rem,4vw,3rem)] lg:text-[clamp(1.75rem,3vw,2.5rem)] xl:text-[clamp(2rem,2.5vw,3rem)] text-white leading-[1.1] sm:leading-[1.15] mb-3 sm:mb-4 group-hover:text-[#DE5127] transition-colors duration-500 text-safe'>{featuredNews.title}</h2>
                                        <p className='font-7 text-[#DE5127] italic text-sm sm:text-base mb-4 sm:mb-6'>{featuredNews.date}</p>
                                        <p className='font-gs text-white/50 text-sm sm:text-base lg:text-lg leading-relaxed mb-6 sm:mb-8 max-w-lg text-safe'>{featuredNews.excerpt}</p>

                                        <motion.div className='self-start flex items-center gap-2 sm:gap-3' whileHover={{ x: 5 }}>
                                            <span className='font-gs text-xs sm:text-sm font-bold uppercase tracking-wider text-white group-hover:text-[#DE5127] transition-colors'>Read Full Story</span>
                                            <div className='w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-white/20 flex items-center justify-center group-hover:border-[#DE5127] group-hover:bg-[#DE5127]/10 transition-all'>
                                                <svg className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-white group-hover:text-[#DE5127] transition-colors' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M17 8l4 4m0 0l-4 4m4-4H3' /></svg>
                                            </div>
                                        </motion.div>
                                    </div>
                                </div>
                        ...
                        <AnimatePresence mode='popLayout'>
                            {regularNews.map((item, index) => (
                                <motion.article key={`${item.id}-${filter}`} layout initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ duration: 0.5, delay: index * 0.1 }} onMouseEnter={() => setHoveredCard(index)} onMouseLeave={() => setHoveredCard(null)} onClick={() => setSelectedArticle(item)} className='group cursor-pointer'>
                                    <div className='relative rounded-2xl overflow-hidden bg-white/[0.02] border border-white/[0.06] group-hover:border-[#DE5127]/30 transition-all duration-500 p-6 sm:p-8'>
                                        {/* Read Time */}
                                        <div className='flex items-center justify-end mb-6 text-white/40'>
                                        ...
                                        <div className='flex items-center justify-between mb-4'>
                                            <span className='font-7 text-[#DE5127] text-sm'>{item.date}</span>
                                        ...
                                        <h3 className='font-2 text-xl sm:text-2xl text-white leading-tight mb-4 group-hover:text-[#DE5127] transition-colors line-clamp-3'>{item.title}</h3>

                                        {/* Excerpt */}
                                        <p className='font-gs text-white/40 text-sm leading-relaxed line-clamp-3 mb-6'>{item.excerpt}</p>

                                        {/* Read More Link */}
                                        <div className='flex items-center gap-2 pt-4 border-t border-white/[0.08] text-white/40 group-hover:text-[#DE5127] transition-colors'>
                                            <span className='font-gs text-[10px] uppercase tracking-wider font-medium'>Read Full Story</span>
            ...
            {/* Newsletter */}
            <section className='px-4 sm:px-8 md:px-12 lg:px-20 xl:px-28 py-12 sm:py-16 lg:py-20 border-t border-white/[0.05]'>
                <div className='max-w-3xl mx-auto'>
                    <motion.div className='flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 p-4 sm:p-6 lg:p-8 rounded-xl sm:rounded-2xl bg-white/[0.02] border border-white/[0.08]' initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
                        <div className='text-center sm:text-left w-full sm:w-auto'>
                            <h3 className='font-2 text-xl sm:text-2xl lg:text-[clamp(1.5rem,3vw,2rem)] text-white mb-1 sm:mb-2'>Join the <span className='text-[#DE5127]'>Hunt</span></h3>
                            <p className='font-gs text-white/40 text-sm sm:text-base'>Early access to drops & rewards</p>
                        </div>
                        <div className='flex flex-col xs:flex-row gap-3 w-full sm:w-auto'>
                            <input type="email" placeholder="Email" className='flex-1 sm:w-48 lg:w-56 bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm text-white placeholder:text-white/30 focus:border-[#DE5127]/50 focus:outline-none font-gs min-w-0' />
                            <motion.button className='px-5 sm:px-6 py-3 bg-[#DE5127] text-white rounded-lg font-gs text-xs sm:text-sm font-bold uppercase tracking-wider hover:bg-white hover:text-black transition-all whitespace-nowrap' whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>Subscribe</motion.button>
                        </div>
                    </motion.div>
                </div>
            </section>
